...
function MoveImages(type) {
  ...
  var slideshowWidth = document.getElementById("slideshow-images-container")
    .clientWidth;
    ...
    if (currentImage === 1) {
      ...
      for (var i = 0; i < images.length; i++) {
        if (images[i].style.left.split("px").length === 1)
          // nếu thuộc tính left không có "px".
          images[i].style.left = images[i].style.left + slideshowWidth + "px";
        else if (images[i].style.left.split("px").length === 2) {
          // nếu thuộc tính left có "px".
          images[i].style.left =
            parseInt(images[i].style.left.split("px")[0]) +
            parseInt(slideshowWidth) +
            "px"; // vd left là "-838px" thì images[i].style.left.split('px') trả về 2 phần tử, đầu tiên là -838, thứ 2 là "px".
        }
      }
      currentImage--;
      ChangeActiveIndicator(currentImage - 1);
...
function MoveToXImage(index) {
  ...
  for (; i < indicators.length; i++) {
    if (indicators[i].classList[0] === "active-indicator") break;
  }
...
function ShowLogin() {
  var login = document.querySelector("#header-right-div .login-info");
  if (login.classList.contains("disappear")) {
